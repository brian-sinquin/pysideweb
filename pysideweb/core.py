...
class QFont:
    ...
    def setItalic(self, italic: bool):
        self._italic = italic
    ...
    def setUnderline(self, underline: bool):
        self._underline = underline

    def to_css(self) -> dict:
        css = {}
        if self._family:
            css["fontFamily"] = self._family
        if self._pointSize > 0:
            css["fontSize"] = f"{self._pointSize}pt"
        if self._pixelSize > 0:
            css["fontSize"] = f"{self._pixelSize}px"
        if self._bold:
            css["fontWeight"] = "bold"
        if self._italic:
    ...
    def __repr__(self):
        return f"QFont('{self._family}', {self._pointSize})"
...
class QTimer:
    ...
    def __init__(self, parent=None):
        ...
        self._parent = parent
        # Bound signal
        self._signal_timeout = BoundSignal(QTimer.timeout, self)

    @property
    def timeout(self):
        return self._signal_timeout

    @timeout.setter
    def timeout(self, value):
        pass  # ignore descriptor override
